          ...
          <div className="flex justify-between items-center">
             <div className="flex items-center space-x-2">
                <h2 className="text-xl font-bold text-white">
                    {activeTab === 'saved' ? 'Saved Items' : (currentUser?.role === 'admin' ? 'Support Inbox' : 'Messages')}
                </h2>
                {currentUser?.role === 'admin' && activeTab !== 'saved' && <Shield className="w-4 h-4 text-red-400" />}
             </div>
             {/* Mobile Close Button */}
          ...
          <div className="flex bg-[#0f1117] p-1 rounded-xl border border-[#2a2e37] space-x-1">
            ...
            <button 
              ...
              className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all flex items-center justify-center space-x-1 ${
                activeTab === 'saved' ? 'bg-amber-500/20 text-amber-400 shadow-sm ring-1 ring-amber-500/20' : 'text-gray-400 hover:text-gray-300'
              }`}
            >
        ...
        <div className="flex-1 overflow-y-auto bg-[#16181d]">
          {activeTab === 'saved' ? (
            <div className="p-4 space-y-4">
              {savedItems.map(item => (
                <div key={item.id} className="bg-[#1e2025] border border-[#2a2e37] rounded-xl p-3 flex gap-3 hover:border-amber-500/30 transition-all cursor-pointer group">
                  <div className="w-20 h-20 bg-[#0f1117] rounded-lg overflow-hidden flex-shrink-0">
                    <img src={item.image} alt={item.title} className="w-full h-full object-cover" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <h4 className="text-sm font-bold text-white truncate mb-1 group-hover:text-amber-400 transition-colors">{item.title}</h4>
                    <p className="text-amber-400 text-sm font-mono font-bold mb-2">{item.price}</p>
                    <div className="flex justify-between items-center">
                      <span className="text-[10px] text-gray-500">{item.location}</span>
                      <button className="text-xs bg-[#2a2e37] hover:bg-[#343944] text-white px-3 py-1 rounded-lg transition-colors">
                        View
                      </button>
                    </div>
                  </div>
      ...
      {activeTab === 'saved' ? (
        <div className="flex-1 bg-[#0f1117] flex flex-col items-center justify-center text-center p-8">
           <div className="w-20 h-20 bg-[#1e2025] rounded-full flex items-center justify-center mb-6">
             <Bookmark className="w-10 h-10 text-amber-400 fill-current" />
           </div>
           <h2 className="text-2xl font-bold text-white mb-2">Saved Items Collection</h2>
           <p className="text-gray-400 max-w-md">Items you bookmark will appear here for quick access. Keep track of your favorite listings easily.</p>
        </div>
      ) : (
