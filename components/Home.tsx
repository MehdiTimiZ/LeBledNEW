    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="flex flex-col items-center justify-center pt-8 pb-6 text-center space-y-8">
        ...
        <div className="w-full max-w-2xl relative group z-10">
          ...
          <div className="relative flex items-center bg-[#181b21] border border-[#2a2e37] rounded-2xl overflow-hidden shadow-2xl focus-within:border-indigo-500 transition-colors">
            <div className="pl-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 items-start">
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <div className="bg-[#13151b] border border-[#2a2e37] rounded-2xl overflow-hidden shadow-lg">
            <div className="p-4 border-b border-[#2a2e37] flex justify-between items-center bg-[#181b21]/50">
              <div className="flex items-center space-x-2">
                ...
                <span className="font-bold text-white text-sm">{t.currencyTitle}</span>
              </div>
            ...
            <div className="p-0">
              <table className="w-full text-xs">
                <thead>
                  <tr className="border-b border-[#2a2e37] text-gray-500 uppercase">
                ...
                <tbody className="divide-y divide-[#2a2e37]">
                  {CURRENCY_RATES.map((rate, idx) => (
                    <tr key={idx} className="hover:bg-[#181b21] transition-colors">
                      <td className="p-3 font-medium text-gray-200">{rate.currency}</td>
          ...
          {/* Events Widget */}
          <div className="bg-[#13151b] border border-[#2a2e37] rounded-2xl p-5 shadow-lg">
             <div className="flex items-center space-x-2 mb-4">
               <Calendar className="w-4 h-4 text-indigo-400" />
               <h3 className="font-bold text-xs text-gray-300 uppercase tracking-wider">{t.eventsTitle}</h3>
             </div>
             <div className="relative pl-2 space-y-6">
                {/* Timeline Line */}
                <div className="absolute left-[7px] top-2 bottom-2 w-0.5 bg-[#2a2e37]"></div>
                
                {events.map((event, i) => (
                   <div key={i} className="relative pl-6 group cursor-pointer" onClick={() => notify(`Event: ${event.title}`, 'info')}>
                      <div className="absolute left-0 top-1 w-3.5 h-3.5 rounded-full bg-[#13151b] border-2 border-indigo-500 group-hover:bg-indigo-500 transition-colors z-10"></div>
                      <p className="text-sm font-medium text-gray-200 group-hover:text-white transition-colors leading-tight mb-1">
                        {event.title}
                      </p>
          ...
          {/* Filters Widget */}
          <div className="bg-[#13151b] border border-[#2a2e37] rounded-2xl p-5 shadow-lg">
             <div className="flex items-center justify-between mb-6 pb-4 border-b border-[#2a2e37]">
               <div className="flex items-center space-x-2">
                 <Filter className="w-5 h-5 text-indigo-400" />
                 <h3 className="font-bold text-lg text-white">{t.filtersTitle}</h3>
               </div>
               <button 
                 onClick={() => {
             ...
             <div className="space-y-6">
                {/* Category Section */}
                <div>
                   <button className="flex items-center justify-between w-full text-xs font-bold text-gray-400 uppercase mb-3 hover:text-gray-200">
                ...
                <div>
                   <button className="flex items-center justify-between w-full text-xs font-bold text-gray-400 uppercase mb-4 hover:text-gray-200">
                   ...
                   <div className="space-y-4">
                      ...
                      <div className="space-y-1.5">
                         ...
                         <div className="relative">
                            <select 
                              ...
                              className="w-full bg-[#0f1117] border border-[#2a2e37] rounded-xl px-3 py-2.5 text-sm text-white focus:outline-none focus:border-indigo-500 appearance-none cursor-pointer"
                            >
                      ...
                      {/* Price */}
                      <div className="space-y-1.5">
                         <label className="text-sm font-medium text-gray-300">{t.price} (DA)</label>
                         <div className="flex gap-2">
                            <input type="text" placeholder="Min" className="w-full bg-[#0f1117] border border-[#2a2e37] rounded-xl px-3 py-2.5 text-sm text-white focus:outline-none focus:border-indigo-500" />
                            <span className="self-center text-gray-500">-</span>
                            <input type="text" placeholder="Max" className="w-full bg-[#0f1117] border border-[#2a2e37] rounded-xl px-3 py-2.5 text-sm text-white focus:outline-none focus:border-indigo-500" />
                         </div>
                      </div>
                   </div>
                </div>
        ...
        <div className="lg:col-span-3">
          <div className="flex items-center justify-between mb-6 bg-[#13151b] p-4 rounded-2xl border border-[#2a2e37]">
            <div className="flex items-center space-x-3">
               <h2 className="text-lg font-bold text-white">{t.resultsFound}</h2>
               <div className="h-4 w-[1px] bg-[#2a2e37]"></div>
            </div>
            <div className="flex items-center space-x-3">
               <span className="text-xs text-gray-500">View:</span>
               <div className="flex bg-[#0f1117] rounded-lg p-1 border border-[#2a2e37]">
                 <button 
                    onClick={() => setViewMode('grid')}
                    className={`p-1.5 rounded transition-all ${viewMode === 'grid' ? 'bg-[#2a2e37] text-white shadow-sm' : 'text-gray-500 hover:text-gray-300'}`}
                    title={t.viewGrid}
                 >
                 ...
                 <button 
                    ...
                    className={`p-1.5 rounded transition-all ${viewMode === 'list' ? 'bg-[#2a2e37] text-white shadow-sm' : 'text-gray-500 hover:text-gray-300'}`}
                    title={t.viewList}
                 >
